...
En este proyecto implementé dos estructuras de datos, una lista doblemente ligadas y un árbol binario:
-Lista Doblemente Ligada: Usada para la búsqueda de un objeto en específico, primero transforma la información del vector Inventario en una lista doblemente ligada, la cual resulta beneficiosa para esta implementación, pues posee navegación bidireccional, lo que le permite moverse hacia adelante y hacia atrás en la lista por medio de punteros, lo que hace que la búsqueda (a pesar de que se mantenga en una complejidad O(n)) resulte eficiente, sobre todo contando que se pueden añadi más datos después.
-Árbol de Búsqueda Binaria: donde, a pesar de que otros árboles como el AVL resulten más eficientes en complejidad (siendo esta de O(log n)), este necesita menos operaciones para mantener el equilibrio en el árbol a la hora de añadir o eliminar objetos, lo que resulta en que, cuando el árbol esté bien distribuido, no necesitará mantener balances, lo que reducirá el costo en memoria de operaciones como añadir o eliminar objetos, cosa que no pasa con los árboles AVL gracias a su estructura de mantenerse siempre balanceados. Igulamente, lo métodos para recorrer el árbol (como inorder, postorder, etc.) son de compeljidad constante, siendo siempre de O(n), donde estas estructuras resultan más eficientes para este árbol que para otro tipo de árboles más grandes que requerirían una estructura de recorridp más compleja. E igualemnte, debido a su estructra binaria, permite que con funciones como "inorder" se ordene rápido y fácilmente el árbol completo. Aparte, contando que en este proyecto se trabajan con pocos datos, resulta más ligero y rápido usarlo para funciones como añadir objeto o eliminarlo. Así que, en conclusión, aprovecho esta estructura de datos debido a su eficiencia y rapidez para trabajar y realizar una gran variedad de operacioens con cantidades pequeñas de datos, lo que sirve muy bien para mi proyecto el cual justamente trabaja con pocos datos.
